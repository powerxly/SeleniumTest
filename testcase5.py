...
class Case_02(unittest.TestCase):
    ...
    def tttttest_flow(self):
        ...
        '''
        #docTitle = "签批（自动测试）%s" % time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))
        docTitle='[政企分公司]签批（自动测试）20171023165354'
        oa_try(self,uihandle,docTitle,resume=20,flow='政企客户分公司呈批件')
        
    def zzztest_flow_01(self):
        '''集团公文-部门通知 20171226
        '''
        uihandle = oa_init()
        
        docTitle = "集团公文-部门通知（自动测试）%s" % time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))
        #docTitle='集团公文-部门通知（自动测试）20171226134227'
        oa_try(self,uihandle,docTitle,resume=0,flow='集团公文-部门通知',retry=3)
    def zzzzztest_flow_02(self):
        '''集团公文-党组办公室文件 20171226
        '''
        uihandle = oa_init()
        
        docTitle = "集团公文-党组办公室文件（自动测试）%s" % time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))
        #docTitle='集团公文-党组办公室文件（自动测试）20171226182359'
        oa_try(self,uihandle,docTitle,resume=0,flow='集团公文-党组办公室文件',retry=5)
    def nnnnnntest_flow_03(self):
        '''集团公文-改革领导小组办公室发文 20171226
        '''
        uihandle = oa_init()
        
        docTitle = "集团公文-改革领导小组办公室发文（自动测试）%s" % time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))
        #docTitle='集团公文-改革领导小组办公室发文（自动测试）20171227101727'
        oa_try(self,uihandle,docTitle,resume=0,flow='集团公文-改革领导小组办公室发文',retry=5)
    def test_flow_04(self):
        '''政企分公司-部门通知 20171226
        '''
        uihandle = oa_init()
        
        #docTitle = "党办文件（自动测试）%s" % time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))
        docTitle='[政企分公司]党办文件（自动测试）20180109111746'
        oa_try(self,uihandle,docTitle,resume=5,flow='政企分公司党办文件',retry=5)
        
    def tttttest_flow2(self):
        uihandle = oa_init()
        #uihandle = ''
        '''
        docTitle = "公文（自动测试）-发文流程%s" % time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))
        ...
        #docTitle='[政企分公司]部门通知（自动测试）20171222160338'
